...
<html>
    ...
    <body onload="cambio=setInterval('regreso()',3000)">
        <%
            ...
            try{
                sta=con.prepareStatement("select Maestro.Id_Maestro from Maestro inner join Datos_Usuario,Datos_Cuenta where Datos_Usuario.Id_Datos_Cuenta=Datos_Cuenta.Id_Datos_Cuenta and Maestro.Id_Datos_Usuario=Datos_Usuario.Id_Datos_Usuario and Datos_Cuenta.Id_Datos_Cuenta=?;");
                sta.setInt(1, (int)(sesion.getAttribute("Id")));
                resul=sta.executeQuery();
                if(resul.next()){
                    String ruta=config.getServletContext().getRealPath("/");
                    sta=con.prepareStatement("select Grupo.Nombre as Grupo,Puntuacion.Cantidad,Escuela.Nombre as Nombre_Escuela,Datos_Usuario.Apellido_Paterno,Datos_Usuario.Apellido_Materno,Datos_Usuario.Nombre as Nombre_Alumno from Puntuacion inner join Escuela,Datos_Usuario,Alumno,Relacion_Maestro_Grupo,Grupo where Grupo.Id_Grupo=Alumno.Id_Grupo and Alumno.Id_Puntuacion=Puntuacion.Id_Puntuacion and Alumno.Id_Datos_Usuario=Datos_Usuario.Id_Datos_Usuario and Alumno.Id_Escuela=Escuela.Id_Escuela and Alumno.Id_Grupo=Relacion_Maestro_Grupo.Id_Grupo and Relacion_Maestro_Grupo.Id_Maestro=? order by Puntuacion.Cantidad desc;");
                    sta.setInt(1, resul.getInt("Id_Maestro"));
